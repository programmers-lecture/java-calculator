...
public class Calculator {
    ...
    public static int calculate(String[] formula) {
        ...
        Stack<Integer> numbers = new Stack<>();         // 숫자 저장 배열
        Stack<String> operator = new Stack<>();         // 사칙연산 부호 저장 배열
        ...
        for(int i=1; i<formula.length; i+=2) {
            if (formula[i].equals("*")) {           // 곱하기 연산이 나오면 저장하지 않고 먼저 계산
                result = multiply(numbers.pop(), Integer.parseInt(formula[i+1]));
                numbers.push(result);               // 계산 결과값 number stack에 저장
            } else if (formula[i].equals("/")) {    // 나누기 연산도 마찬가지로 저장하지 않고 먼저 계산
                result = divide(Integer.parseInt(formula[i-1]), Integer.parseInt(formula[i+1]));
                numbers.push(result);                // 계산 결과값 number stack에 저장
            } else {            // 더하기, 빼기 연산자가 나올 경우 일단 number stack, operator stack에 저장
                numbers.push(Integer.parseInt(formula[i+1]));
        ...
        for(int i=0; i<operator.size(); i++) {
            if (operator.get(i).equals("+")) {
                result = add(numbers.pop(), numbers.pop());
                numbers.push(result);
            } else {
                result = subtract(numbers.pop(), numbers.pop());
                numbers.push(result);
            }
        }
        return result;
